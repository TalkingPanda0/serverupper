use std::{io::{BufReader, BufWriter, Write}, net::{TcpStream, UdpSocket}};

use anyhow::Result;

use crate::{packet::Packet, reader::Reader, writer::Writer};

pub fn ping_server(address: &str, port: u16) -> Result<Packet> {
    let stream = TcpStream::connect(format!("{address}:{port}"))?;

    let mut writer = BufWriter::new(&stream);
    let mut reader = BufReader::new(&stream);

    writer.write_packet(&Packet::Handshake { version: 776, address: address.into(), port, state: 1 })?;
    writer.write_packet(&Packet::StatusRequest)?;
    writer.flush()?;

    reader.read_clientbound_packet()
}
